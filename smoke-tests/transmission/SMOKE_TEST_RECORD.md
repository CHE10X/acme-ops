...
| 2 Version | FAIL | `transmission_router.py --version` → returns operation JSON (`{"status":"EXHAUSTED",...}`) instead of version string. `__version__ = "2.0.0"` exists in source but `--version` flag not wired. |
